defmodule FungusToast.ActiveCellChangesTest do
  ...
  describe "update_active_cell_changes/4" do
    test "that it returns true when there was an empty map" do
      assert ActiveCellChanges.update_active_cell_changes(%Player{}, -1, 0, %{})
    end
    ...
    test "it returns true when the minimum round is met for the skill chosen" do
      user = Fixtures.Accounts.User.create!()
      game = Games.create_game(user.user_name, %{number_of_human_players: 1})
      player = hd(Players.list_players_for_game(game.id))
      ...
      list_with_max_allowed_changes = Enum.map(1..skill.number_of_toast_changes, fn x -> x end)
      upgrade_attrs = %{skill.id=> %{"active_cell_changes" => list_with_max_allowed_changes, "points_spent" => 1}}
      assert ActiveCellChanges.update_active_cell_changes(player, game.id, ActiveSkills.minimum_number_of_rounds_for_dead_cell(), upgrade_attrs)
      updated_player = Players.get_player!(player.id)
      assert updated_player.action_points == 0
    end
    ...
    test "it returns false when the player doesn't have enough action points" do
      ...
      refute ActiveCellChanges.update_active_cell_changes(player, game.id, 0, upgrade_attrs)
    end
    ...
    test "it returns false when there are active cell changes but no points are spent" do
      ...
      refute ActiveCellChanges.update_active_cell_changes(%Player{}, -1, 0, upgrade_attrs)
    end
    ...
    test "it returns false when there are more active cell changes than the max allowed for that skill" do
      ...
      refute ActiveCellChanges.update_active_cell_changes(%Player{action_points: 1}, -1, 0, upgrade_attrs)
    end

    test "it returns false when the minimum round is not met for the skill chosen" do
      user = Fixtures.Accounts.User.create!()
      game = Games.create_game(user.user_name, %{number_of_human_players: 1})
      player = hd(Players.list_players_for_game(game.id))
      skill = ActiveSkills.get_active_skill!(ActiveSkills.skill_id_dead_cell())
      list_with_max_allowed_changes = Enum.map(1..skill.number_of_toast_changes, fn x -> x end)
      upgrade_attrs = %{skill.id=> %{"active_cell_changes" => list_with_max_allowed_changes, "points_spent" => 1}}
      refute ActiveCellChanges.update_active_cell_changes(player, game.id, ActiveSkills.minimum_number_of_rounds_for_dead_cell() - 1, upgrade_attrs)
    end
